...
namespace JMiles42.Maths
{
	...
	public static class CreateColours
	{
		...
		public static Color BuildSingleColour255 (byte value, ColorRGBAType colType)
		{
			Color c = new Color (0f, 0f, 0f, 1f);
			switch ( colType )
			{
				case ColorRGBAType.Red:
					c.r = value == 0 ? 0 :value / 255;
					return ( c );
				case ColorRGBAType.Green:
					c.g = value == 0 ? 0 : value / 255;
					return ( c );
				case ColorRGBAType.Blue:
					c.b = value == 0 ? 0 : value / 255;
					return ( c );
				case ColorRGBAType.Alpha:
		...
		/// <param name="colType"> Lets you specify a colour type. </param>
		/// <returns> The built color from values passed to it </returns>
		public static Color BuildSingleColour (float value, ColorRGBAType colType)
		{
			Color c = new Color (0f, 0f, 0f, 1f);
			switch ( colType )
			{
		...
		/// <returns>The built color from values passed to it</returns>
		public static Color BuildColorWithoutType255(int valueOne, int valueTwo, int valueThree, ColorRGBAType colType)
		{
			Color c = new Color (0f, 0f, 0f, 1f);
			switch ( colType )
			{
		...
		/// <param name="colType"> Lets you specify a colour type to exclude. </param>
		/// <returns>The built color from values passed to it</returns>
		public static Color BuildColorWithoutType (float valueOne, float valueTwo, float valueThree, ColorRGBAType colType)
		{
			Color c = new Color (0f, 0f, 0f, 1f);
			switch ( colType )
			{
		...
		public static Color BuildColourInt255 (byte valueRed, byte valueGreen, byte valueBlue, byte valueAlpha = 255)
		{
			Color c = new Color 
				(
                     valueRed == 0 ? 0 :valueRed / 255,
                     valueGreen == 0 ? 0 :valueGreen / 255,
                     valueBlue == 0 ? 0 :valueBlue / 255,
                     valueAlpha == 0 ? 0 :valueAlpha / 255
                );
			return ( c );
		}
		#endregion
		...
		{
			Color c = new Color (valueRed, valueGreen, valueBlue, valueAlpha);
			return ( c );
		}
		#endregion
	}
