...
namespace JMiles42.Physics
{
    public class PhysicalObject : JMilesBehaviour
    {
        ...
        public Vector3 finalVelocity
        {
            get
            {
                if (useWorldGravity) return (velocity) + ( useGravityFromObjects?velocityOthers:Vector3.zero) + DownGravity * acceleration * mass;
        ...
        private void OnDrawGizmosSelected()
        {
            ...
            if (finalVelocity.magnitude != 0f)
                Handles.ArrowCap(0, globalCenterOfMass, Quaternion.LookRotation(finalVelocity * 360),
                    finalVelocity.magnitude / 10);
        }

        public bool VectorCollidingWithObject(Vector3 posToCheck)
        {
            if(collider) return collider.VectorCollidingWithObject(posToCheck);
